...
*******************************************************************************/
#define ASCII2HEX(c, uc)  { if ((c >='0') && (c <='9')) uc = c - '0'; \
		else if ((c >='a') && (c <='f')) uc = c - 'a' + 0xA; \
		else if ((c >='A') && (c <='F')) uc = c - 'A' + 0xA; \
		else uc = 0; }
	...
	if (strncmp(c->cmd, "daq", 3) == 0)
	{
		...
		*((volatile unsigned int *) (XPAR_AXI_BRAM_CTRL_4_S_AXI_BASEADDR + 0x04)) = *((volatile unsigned int *) (XPAR_AXI_BRAM_CTRL_4_S_AXI_BASEADDR + 0x04)) ^ R1_TIME_PROBE;
		...
		// "daq 0xhhhhhh C"
		{
			...
			{
				p+=3;
				ASCII2HEX(*(p), uc); param[0] = param[0] | (uc << 20); p++;
				ASCII2HEX(*(p), uc); param[0] = param[0] | (uc << 16); p++;
				ASCII2HEX(*(p), uc); param[0] = param[0] | (uc << 12); p++;
				ASCII2HEX(*(p), uc); param[0] = param[0] | (uc <<  8); p++;
				ASCII2HEX(*(p), uc); param[0] = param[0] | (uc <<  4); p++;
				ASCII2HEX(*(p), uc); param[0] = param[0] | (uc      ); p++;
				err++;
			}
			if (*(p) == ' ')
			{
			...
			{
				p+=3;
				ASCII2HEX(*(p), uc); req_seq = req_seq | (uc <<  4); p++;
				ASCII2HEX(*(p), uc); req_seq = req_seq | (uc      ); p++;
				err++;
			}
		}
