...
int CmdFetcher_Main(CmdFetcher *cf)
{
	...
	while (!alldone)
	{
			...
			while (!cmd_stdin_done)
			{
				...
				if (strncmp(cmd_stdin, "exec ", 5) == 0)
				{
					...
					if( strcmp( cf->cmd_file, "ped" ) != 0 &&  strcmp( cf->cmd_file, "start" ) != 0 )
					{
						...
						{
							sprintf( runTagStr, "Test" );
							sprintf( driftFieldStr, "0" );
							sprintf( meshVoltageStr, "0" );
							sprintf( detectorPressureStr, "0" );
						}
						...
						if( savePed )
						{
							...
							sprintf(tmpStr, "cp ped %s/R%05d_%s_Vm_%s_Vd_%s_Pr_%s_Gain_%s_Shape_%s_Clock_%s.ped",
									getenv( "GOOD_DATA_PATH" ), runNumber, runTagStr, meshVoltageStr, driftFieldStr,
									detectorPressureStr, gainStr, shapingStr, clockStr );
							if( dataTakingMode == 0 )
							sprintf(tmpStr, "cp ped %s/R%05d_%s_Vm_%s_Vd_%s_Pr_%s_Gain_%s_Shape_%s_Clock_%s.ped",
									getenv( "TEST_DATA_PATH" ), runNumber, runTagStr, meshVoltageStr, driftFieldStr,
									detectorPressureStr, gainStr, shapingStr, clockStr );
							system( tmpStr );
							savePed = 0;
						}
						if( savePed && dataTakingMode == 0 )
						{
							sprintf(tmpStr, "cp ped %s/R%05d_%s_Vm_%s_Vd_%s_Pr_%s_Gain_%s_Shape_%s_Clock_%s.ped",
									getenv( "TEST_DATA_PATH"), runNumber, runTagStr, meshVoltageStr, driftFieldStr,
									detectorPressureStr, gainStr, shapingStr, clockStr );
		...
		else if (strncmp(cmd, "event_builder", 13) == 0)
		{
			...
			printf("%s.rep(?): Event Builder Mode: %s (0x%x)\n", fem_tar_str, &EventBuilder_Mode2str[eb->eb_mode][0], eb->eb_mode);
			post_cmd = 0;
		}
		// File chunk size
		...
		else if (strncmp(cmd, "program flash", 13) == 0)
		{
			...
			{
				printf("%s.rep(?): file format must be \".mcs\" not \"%s\"\n", fem_tar_str, &cf->cmd_file[strlen(cf->cmd_file)-4]);
				err = -1;
				cf->no_echo_this = 1;
			}
